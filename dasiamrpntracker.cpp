...
//    std::cout << output.cfg.anchor.col(0);
    ...
    caffe2::Tensor<CPUContext>* feature_out = new caffe2::Tensor<CPUContext>();
    this->GetTensorToHost(this->trackerEngine->GetBlob("feature_out")->template GetMutable<caffe2::Tensor<CUDAContext>>(),feature_out);
...
//    std::cout << caffe2::EigenMatrixMap<float>(roi.data(),feature_out->size(),1) << std::endl;
    ...
    if(this->mode == caffe2::CUDA){
        ...
        auto conv_r1 = this->trackerEngine->GetBlob(ss)->template GetMutable<caffe2::Tensor<CUDAContext>>();
        auto kernel_size = int(sqrt(conv_r1->size()/(4*512*this->trackInfo.cfg.anchor_num)));
        ss = "conv_cls1";
        auto conv_cls1 = this->trackerEngine->GetBlob(ss)->template GetMutable<caffe2::Tensor<CUDAContext>>();
        ...
        std::string ss = "conv_r1";
        auto conv_r1 = this->trackerEngine->GetBlob(ss)->template GetMutable<caffe2::Tensor<CPUContext>>();
        auto kernel_size = int(sqrt(conv_r1->size()/(4*512*this->trackInfo.cfg.anchor_num)));
        ss = "conv_cls1";
        auto conv_cls1 = this->trackerEngine->GetBlob(ss)->template GetMutable<caffe2::Tensor<CPUContext>>();
    ...
    this->GetTensorToHost(this->trackerEngine->GetBlob("r2_out_w")->template GetMutable<caffe2::Tensor<CUDAContext> >(),r2_out_w);
    caffe2::TensorCPU* r2_out_b = new caffe2::TensorCPU();
    this->GetTensorToHost(this->trackerEngine->GetBlob("r2_out_b")->template GetMutable<caffe2::Tensor<CUDAContext> >(),r2_out_b);
...
//#if 0
    ...
    if(this->mode == caffe2::CUDA){
        caffe2::TensorCUDA* r2_premute_CUDA = this->trackerEngine->GetBlob("r2_permute_out")->template GetMutable<caffe2::Tensor<CUDAContext> >();
        ...
        caffe2::TensorCUDA* cls2_premute = this->trackerEngine->GetBlob("cls2_permute_out")->template GetMutable<caffe2::Tensor<CUDAContext> >();
        ...
        caffe2::TensorCUDA* score_res_CUDA = this->trackerEngine->GetBlob("softmax_tranpose_out")->template GetMutable<caffe2::Tensor<CUDAContext> >();

        this->GetTensorToHost(score_res_CUDA,score_res);
    }else{
        r2_premute = this->trackerEngine->GetBlob("r2_permute_out")->template GetMutable<caffe2::Tensor<CPUContext> >();
        r2_premute->Reshape(std::vector<int>{4,r2_premute->size()/4});
        static auto cls2_op = caffe2::CreateOperatorDef("Transpose",
                                                      "cls2_permute",
                                                      {"cls_out"},{"cls2_permute_out"},{arg},this->devOption);
        this->trackerEngine->RunOperatorOnce(cls2_op);
        caffe2::TensorCPU* cls2_premute = this->trackerEngine->GetBlob("cls2_permute_out")->template GetMutable<caffe2::Tensor<CPUContext> >();
        cls2_premute->Reshape(std::vector<int>{2,cls2_premute->size()/2});
        caffe2::Argument arg_transpose;
        arg_transpose.add_ints(1);
        arg_transpose.add_ints(0);
        ...
        static auto softmax_transpose_op = caffe2::CreateOperatorDef("Transpose",
                                                      ...
                                                      {"cls2_softmax_out"},{"softmax_tranpose_out"},{arg_transpose},this->devOption);
        this->trackerEngine->RunOperatorOnce(softmax_transpose_op);
        score_res = this->trackerEngine->GetBlob("softmax_tranpose_out")->template GetMutable<caffe2::Tensor<CPUContext> >();
    }
    ...
    this->GetTensorToHost(this->trackerEngine->GetBlob("cls2_permute_out")->template GetMutable<caffe2::Tensor<CUDAContext> >(),cls2_permute_out);
    caffe2::TensorCPU* cls2_softmax_out = new caffe2::TensorCPU();
    this->GetTensorToHost(this->trackerEngine->GetBlob("softmax_tranpose_out")->template GetMutable<caffe2::Tensor<CUDAContext> >(),cls2_softmax_out);
...
void DaSiamRPNTracker::regress_adjust()
{
    ...
    if(this->mode == caffe2::CUDA){
        auto r1_kernel = this->trackerEngine->GetBlob("r1_kernel")->template GetMutable<caffe2::Tensor<CUDAContext> >();
        auto cls1_kernel = this->trackerEngine->GetBlob("cls1_kernel")->template GetMutable<caffe2::Tensor<CUDAContext> >();
        ...
        auto r1_kernel = this->trackerEngine->GetBlob("r1_kernel")->template GetMutable<caffe2::Tensor<CPUContext> >();
        auto cls1_kernel = this->trackerEngine->GetBlob("cls1_kernel")->template GetMutable<caffe2::Tensor<CPUContext> >();
    ...
    this->GetTensorToHost(this->trackerEngine->GetBlob("r2_out")->template GetMutable<caffe2::Tensor<CUDAContext> >(),r2_out);
    caffe2::TensorCPU* cls_out = new caffe2::TensorCPU();
    this->GetTensorToHost(this->trackerEngine->GetBlob("cls_out")->template GetMutable<caffe2::Tensor<CUDAContext> >(),cls_out);
